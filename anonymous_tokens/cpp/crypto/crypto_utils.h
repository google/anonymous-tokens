...
absl::StatusOr<std::string>  BignumToString(
...
std::string  EncodeMessagePublicMetadata(
    absl::string_view message, absl::string_view public_metadata);

// Compute 2^(x - 1/2).
absl::StatusOr<bssl::UniquePtr<BIGNUM>>  GetRsaSqrtTwo(
    int x);
...
//
absl::StatusOr<std::string>  RsaSsaPssPublicKeyToDerEncoding(
    const RSA* rsa);
